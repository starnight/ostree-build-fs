...
apk add --no-cache ostree multipath-tools sfdisk
...
PARTUUID=$(blkid -s UUID ${root} | cut -d '"' -f2)
ostree admin deploy --sysroot="${TARGET_DIR}" --os=${OSNAME} --karg="root=UUID=${PARTUUID}" --karg=rw ${OSTREE_REMOTE_NAME}:${OSTREE_BRANCH}
